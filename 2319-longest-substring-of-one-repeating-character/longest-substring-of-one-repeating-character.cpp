...
class SegmentTree {
    ...
    void pull(int node, int l, int r) {
        ...
        if (tree[left].rightChar == tree[right].leftChar)
            tree[node].best = max(tree[node].best, tree[left].rightLen + tree[right].leftLen);
    }
...
public:
    ...
    void update(int pos, char ch) { update(1, 0, n-1, pos, ch); }
    int getBest() const { return tree[1].best; }
};
